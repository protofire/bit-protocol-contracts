...
contract AllocationVesting is DelegatedOps {
    ...
    error SelfTransfer();
    error IncompatibleVestingPeriod(
        uint256 numberOfWeeksFrom,
        uint256 numberOfWeeksTo
    );
    ...
    constructor(
        ...
        for (uint256 i; i < loopEnd; ) {
            ...
            if (allocations[recipient].numberOfWeeks > 0)
    ...
    function transferPoints(
        ...
        address to,
        uint256 points
    ) external callerOrDelegated(from) {
        ...
        if (numberOfWeeksTo != 0 && numberOfWeeksTo != numberOfWeeksFrom)
            revert IncompatibleVestingPeriod(
                numberOfWeeksFrom,
                numberOfWeeksTo
            );
        uint256 totalVested = _vestedAt(
            block.timestamp,
            pointsFrom,
            numberOfWeeksFrom
        );
        ...
        uint256 claimed = _claim(
            from,
            pointsFrom,
            fromAllocation.claimed,
            numberOfWeeksFrom
        );
        // Passive balance to transfer
        uint128 claimedAdjustment = uint128(
            (claimed * points) / fromAllocation.points
        );
        ...
        uint256 preclaimedToTransfer = (fromAllocation.preclaimed * points) /
            pointsFrom;
        allocations[to].preclaimed = uint96(
            toAllocation.preclaimed + preclaimedToTransfer
        );
        allocations[from].preclaimed = uint96(
            fromAllocation.preclaimed - preclaimedToTransfer
        );
    ...
    function lockFutureClaimsWithReceiver(
        ...
        uint256 userAllocation = (allocation.points * totalAllocation) /
            totalPoints;
        ...
        else if (preclaimed + amount > maxTotalPreclaim || amount > _unclaimed)
            revert PreclaimTooLarge();
    ...
    function _vestedAt(
        ...
        vested =
            (totalAllocation * timeSinceStart * points) /
            (totalPoints * vestingWeeks);
    }
    ...
    function unclaimed(address account) external view returns (uint256) {
        AllocationState memory allocation = allocations[account];
        uint256 accountAllocation = (totalAllocation * allocation.points) /
            totalPoints;
